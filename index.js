...
// Codes ANSI simples pour les logs
const colors = {
  reset: "\x1b[0m",
  bright: "\x1b[1m",
  dim: "\x1b[2m",
...
class SomfyGatePlatform {
  constructor(log, config, api) {
    ...
    if (api) {
      this.api.on('didFinishLaunching', async () => {
        ...
        // Création de l'accessoire si deviceURL défini
  ...
  configureAccessory(accessory) {
    ...
    // Si deviceURL n'est pas défini ou ne correspond pas, on ignore l'accessoire
    if (!this.config.deviceURL || uuid !== this.api.hap.uuid.generate(this.config.deviceURL)) {
      this.log.info(`[Portail] Ignoré accessoire orphelin : ${accessory.displayName}`);
      return;
    }
  ...
  async createAccessory() {
    ...
    if (this.registeredUUIDs.has(uuid)) {
      this.log.info("[Portail] Accessoire déjà enregistré, aucune action nécessaire.");
      return;
    }
    ...
    // Stop (optionnel, peut être réactivé)
      ...
      setInterval(async () => {
        ...
        switch (state.currentDoorState) {
          case Characteristic.CurrentDoorState.CLOSED:
            txtState = "Fermé"; break;
          case Characteristic.CurrentDoorState.OPEN:
            txtState = "Ouvert"; break;
          case Characteristic.CurrentDoorState.STOPPED:
            txtState = "Arrêté / Inconnu"; break;
          case Characteristic.CurrentDoorState.OPENING:
            txtState = "Ouverture en cours"; break;
          case Characteristic.CurrentDoorState.CLOSING:
            txtState = "Fermeture en cours"; break;
        }
        this.log.info(`${colors.cyan}[Portail] État actuel : ${txtState}`);
      }, interval);
    }
  }
