...
class SomfyGatePlatform {
  constructor(log, config, api) {
    ...
    if (api) {
      this.api.on('didFinishLaunching', async () => {
        ...
        // Ici on crée l’accessoire dynamique
        ...
        const garageService = accessory.addService(Service.GarageDoorOpener, this.config.name);
        ...
        // Publication dynamique dans HomeKit ✅
        this.api.publishExternalAccessories("homebridge-somfy-tahoma-v2-gate", [accessory]);
  ...
  // Nécessaire en Dynamic Platform ✅
  configureAccessory(accessory) {
    this.log("Chargement de l’accessoire depuis le cache :", accessory.displayName);
  
    // Si l’accessoire correspond bien à l’URL configurée → on le garde
    if (this.config.deviceURL && accessory.UUID === this.api.hap.uuid.generate(this.config.deviceURL)) {
      this.log("➡️ Accessoire conservé :", accessory.displayName);
      this.accessoriesList.push(accessory);
    } else {
      // Sinon, on le supprime du cache
      this.log.warn("❌ Suppression de l’accessoire orphelin :", accessory.displayName);
