    ...
    async discoverDevices() {
        try {
            ...
            for (const device of devices) {
               // Inclure uniquement les équipements de type portail
		const isGate = ['Gate', 'GarageDoor'].includes(device.definition.uiClass) ||
	               ['GarageDoorOpener', 'GateOpener'].includes(device.definition.widgetName);

		if (!isGate) {
		    this.log.debug(`Ignore non-gate device: ${device.label} (${device.definition.uiClass} / ${device.definition.widgetName})`);
		    continue;
		}
